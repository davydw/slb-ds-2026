...
- Run the code with `python -m farm.farming_diary`. Solve one error at a time by coding the missing `talk` methods in `Cow` and `Chicken`.
...
Once it works, run `make` to test your code. By now you should pass 12 tests and fail 1. We'll solve that later.

Time to commit and push the code you created. Run `git status` to check which files you modified. Then commit and push.
...
When your program works, let's run `make` again to test if you coded it properly.

If all went well, you should still pass 12 tests, and have 1 failed test.

Try to find out what the problem is for this last test. Why is it failing?

<details>
  <summary markdown='span'>💡 Hints</summary>

  We wrote a test to check if you have tests for the `Cow` class. And no, there aren't any ... yet.
...
When you're ready, run `make` again and see how the additional tests get executed. You should now have 19 passed tests.
