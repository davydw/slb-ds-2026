...
Run the following command to download the file into the `data` folder:

```bash
curl https://wagon-public-datasets.s3.amazonaws.com/02-Data-Toolkit/02-Data-Sourcing/receipts.txt > data/receipts.txt
```
...
**Regex (Regular Expressions)** is a powerful tool for pattern matching and text manipulation. It can replace Python string methods like split(), replace(), and find() to simplify complex operations. It allows you to search, extract, and modify text efficiently using concise patterns. Regex is not limited to Python: it exists in many programming languages like Ruby, JavaScript, Java, Rust, ...
...
[re.findall( ) documentation](https://docs.python.org/3/library/re.html#re.findall)
...
Do that in your notebook and make sure you can see some of the receipts.
...
You can look at [this documentation](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html) if you don't remember the syntax.
...
To do the conversion you can use the [`Pandas.to_datetime()` documentation](http://pandas.pydata.org/docs/reference/api/pandas.to_datetime.html) and [the strftime format documentation](http://strftime.org/)
...
You can use the DataFrame method [`set_index()` documentation](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.set_index.html)
<details><summary markdown='span'>Solution
</summary>
...
To do the conversion you can use the [`pandas.to_numeric()` documentation](http://pandas.pydata.org/docs/reference/api/pandas.to_numeric.html)
