...
Once the project is created, you'll see a **project ID**. This ID is important because it will be referenced throughout the rest of the setup.
...
1. You need the Google Cloud SDK. We already installed it on setup day, so no need to do this now. (If you ever need to install it on a new machine, you can find the instructions [here](https://cloud.google.com/sdk/docs/install).)

2. Authenticate your GCP account (in your terminal):

   ```bash
   gcloud auth login
   ```

2. Set your new project as the active project using your project ID:
...
#### 2.2.1 📥 Install pyenv
We already installed pyenv during setup day, so no need to do it now. (If you ever need to install it on a new machine, follow the installation instructions on the official pyenv GitHub repository. This typically involves running a few commands in your terminal to set up pyenv on your system.)
...
Use pyenv to install the specific version of Python that your project requires. You don't need to do this now, because we'll use the same Python version as in the Le Wagon setup.

For example, to install Python 3.10.6, you would use:
...
Explanation: This command creates a virtual environment named myenv using Python 3.10.6. The virtual environment will be tied to this specific Python version.
...
Now anytime you move into your project folder, pyenv will automatically use your project's virtual environment.
...
Before starting, open the notebook file in the `notebooks` folder (`my_notebook.ipynb`). Review its contents to understand the structure and any relevant code or data. This will give you context and set the foundation for the following steps. 🚀

Once familiar, run the notebook to ensure everything works properly. Do you get any `Module not found` errors? That's because the Python packages your project needs haven't been installed: we started from a fresh environment with no packages. Install the packages you need with `pip install <package-name>`, for example `pip install scikit-learn`.

If all went well, your notebook will have stored a trained model in a `.pkl` file.
...
2. Load the Machine Learning model `.pkl` file, the one exported by the notebook.
...
<details>
<summary markdown="span">🔍 Click here to see the solution</summary>
...
3. If you have already installed packages manually using `pip install`, you can generate this file automatically by running the following command in your terminal:
    ```bash
...
3. **Packages**: You’ll also need to specify which packages should be included when your package is installed. This can be done using `find_packages()` to automatically find and include all relevant Python modules and sub-packages within your project. This is where the `__init__.py` file that we created earlier comes into play. 📚
...
This will install the package in your current folder (`.`) in editable mode (`-e`), meaning any changes made to the code will be reflected without needing to reinstall the package. 🔄
...
This structure allows you to easily extend and reuse the function in various locations within your project folder. Try it out if you don't believe me 😉. It also allows you to reuse the function in various parts of your application, such as within a FastAPI project. 🌐
...
To shut down the process, hit `ctrl + c` in your terminal.
...
1. **Base Image**: Specify a base image that includes Python 3.10 (because that's the version we used to develop). Use a slim version to keep the image size small.

2. **Copy Package Folder**: Add a command to copy your project's `package_folder` into the Docker image. This should include all your application code.
...
1. `FROM python:3.10-slim`: This specifies the base image we're building from. In this case, it's a slim version of Python 3.10.

1. `COPY requirements.txt requirements.txt`: This copies your requirements.txt file into the Docker image.

1. `RUN pip install -r requirements.txt`: This command installs all the Python packages listed in your requirements.txt file.

1. `COPY package_folder package_folder`: This copies your project's package folder into the Docker image.

1. `COPY models models`: This copies your models folder into the Docker image.

1. `CMD uvicorn package_folder.api_file:app --host 0.0.0.0`: This specifies the command to run when the container starts. It starts the Uvicorn server to run your FastAPI application.
...
During the Le Wagon setup, we setup direnv so that it will automatically reload after every command you run in the terminal (or simply hit RETURN on an empty line).
...
- `-p 8080:8000`: Maps port 8080 of your machine to port 8000 inside the container, allowing you to access the app locally via `http://localhost:8080`.
...
Explanation:
- This command builds a Docker image suitable for deployment
- It builds the image with a longer tag, using the `GCP_REGION`, `GCP_PROJECT`, `ARTIFACTSREPO` and `IMAGE` environment variables from the `.env` file. (Ensure that those environment variables are correctly set in your `.env` file.)
- It ensures that it builds an image that can run on a linux environment on a classic AMD or Intel processor. This is needed if you are using an ARM-based machine like Macs running Apple Silicon M1 or more recent processors.

#### 5.6.2 📤 Push to Container Registry

Before pushing your production image to the container registry, follow these steps to fully authenticate and grant all necessary permissions:
...
**2. Add Permissions to Your Google Cloud Account:**

Before creating the Artifact Registry, ensure you have the necessary permissions. Run the following command to add the `Artifact Registry Writer` role to your account. Replace `YOUR_EMAIL` with your email address:
...
**3. Create an Artifact Registry Repository using .env Variables:**
...
gcloud artifacts repositories create $ARTIFACTSREPO \
  ...
  --location=$GCP_REGION \
  --description="My Docker Artifact Repository"
```
...
If the process worked, you should see a message in your terminal signaling that the Service has been deployed and is serving 100 percent of traffic. You should also be provided with a Service URL, the one allowing anyone in the world to connect to your API 🤯. Now go and try out your API in the cloud by browsing to this URL.
...
We create a separate folder, because the frontend is a separate project: it doesn't need any of the machine learning libraries we used for the backend. The only thing it needs to do is connect to the API we created before. You could even ask a web developer to create your frontend in another programming language like Ruby or JavaScript.
...
### 6.2 🐍 Setting Up a Separate Virtual Environment

Because we'll only need a few libraries, much less than for our backend, we setup a separate lightweight virtual environment. This will make it very easy to deploy it in production.

You know the drill from the backend:

```bash
pyenv virtualenv 3.10.6 frontenv
pyenv local frontenv
```
...
<details>
<summary markdown="span">🔍 Click here to see a solution example</summary>
...
Notice how we are not using scikit-learn in this file! As said before, our frontend doesn't need any machine learning libraries. It is a very lightweight web application.
...
1. **Install GitHub CLI and login to GitHub via the CLI**
   You don't need to do this now because we already installed and logged in during the bootcamp setup.

   If you ever need to do it, these are the steps:
...
3. **Add Your Files to the Git Repository**
   ...
   - Add your project files to the Git staging area:
     ...
     ```bash
     git add frontend_file.py requirements.txt
     ```
...
4. **Create a GitHub Repository from the Terminal**
...
6. **Push Your Code to GitHub**
   - Now that the repository is created and linked, push your code to GitHub (replace `main` by `master` if you're default branch was created as `master`):
...
3. Select the branch where your `frontend_file.py` is located (`main` or `master`) and click "Deploy."
